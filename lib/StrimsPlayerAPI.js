...
StrimsPlayerAPI.prototype._call = function(method, options, callback) {
    ...
    .end( function(res) {
        if (res.error) {
            ...
            callback({status: 500, message: JSON.stringify(res.error)});
        }
        if (res.status != 200) {
            console.error('StrimsPlayerAPI/_call error response status=%s body=%s',res.status, JSON.stringify(res.body));
            var error = {status: res.status, message: JSON.stringify(res.body)};
...
StrimsPlayerAPI.prototype.addSong = function(data, callback) {
    if (typeof data === 'Array') {
        this._call(StrimsPlayerAPI.API.ADD_SONGARRAYS, {method: 'POST', data: data}, callback);
    } else {
        this._call(StrimsPlayerAPI.API.ADD_SONG, {method: 'POST', data: data}, callback);
    }
};

StrimsPlayerAPI.API = {};
StrimsPlayerAPI.API.ADD_SONG = '/api/song/add';
StrimsPlayerAPI.API.ADD_SONGARRAY = '/api/song/addarray';
